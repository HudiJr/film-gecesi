...
// Mobilde kamera erişilememe/izin reddi durumunda çökmemesi için yedek akış üretici (Fallback Stream)
...
landingCreateBtn.addEventListener('click', async () => {
    ...
    if (!myPeerId) {
        alert('Bağlantı kuruluyor, lütfen 1 saniye bekleyip tekrar deneyin.');
        return;
    }
    await startCamera();
...
async function startCamera(isSwitch = false) {
    try {
        ...
        console.warn('Kamera/Mikrofon erişilemedi, yedek akışa geçiliyor:', err);
        localStream = createFallbackStream();
        localVideo.srcObject = localStream;
    }
}
...
socket.on('user-connected', ({ peerId, username, avatar }) => {
    if (peerId) setTimeout(() => connectToNewUser(peerId, username, avatar, localStream), 1000);
});
