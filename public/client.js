...
async function startCamera(isSwitch = false) {
    try {
        ...
        if (isSwitch && currentRoom) {
            // Kameralar değiştiğinde bağlı olan diğer peer'lara yeni stream'i gönder
...
// YouTube Arama / Link işleme (Artık site içine gömülü açılıyor!)
ytLoadBtn.addEventListener('click', () => {
    ...
    if (videoId) {
        ...
        // YouTube arama sonuç sayfasını iframe içinde siteye gömüyoruz
        targetUrl = `https://www.youtube.com/results?search_query=${encodeURIComponent(inputVal)}`;
    }

    socket.emit('change-yt-video', { roomId: currentRoom, url: targetUrl });
...
socket.on('yt-video-changed', ({ url }) => {
    mainVideo.style.display = 'none';
    googlePlayer.style.display = 'none';
    noStreamPlaceholder.style.display = 'none';
