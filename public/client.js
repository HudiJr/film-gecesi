...
// YouTube Modal & Arama Kontrolleri
...
ytLoadBtn.addEventListener('click', () => {
    ...
    if (!inputVal || !currentRoom) return alert('Lütfen link veya arama terimi girin!');

    const videoId = extractYouTubeId(inputVal);
    if (videoId) {
        socket.emit('change-yt-video', { roomId: currentRoom, type: 'video', value: videoId });
    } else {
        // Doğrudan YouTube arama sonuçları URL'si oluşturup iframe'e veriyoruz
        const searchUrl = `https://www.youtube.com/results?search_query=${encodeURIComponent(inputVal)}`;
        socket.emit('change-yt-video', { roomId: currentRoom, type: 'search', value: searchUrl });
    }
...
socket.on('yt-video-changed', ({ type, value }) => {
    ...
    if (type === 'video') {
        ...
        ytPlayer.src = value; // YouTube arama sonuçları sayfası
    }
});

// Google Modal & Arama Kontrolleri
...
googleLoadBtn.addEventListener('click', () => {
    ...
    if (!query || !currentRoom) return alert('Lütfen bir arama terimi veya adres girin!');
