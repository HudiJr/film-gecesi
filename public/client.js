...
// Kaydırma senkronizasyonu için kilit değişkeni
...
ytLoadBtn.addEventListener('click', async () => {
    ...
    if (videoId) {
        ...
        alert('Aradığınız kriterlere uygun video bulunamadı. Lütfen doğrudan YouTube video linki yapıştırın!');
    }
});
...
googleLoadBtn.addEventListener('click', () => {
    ...
    // Doğrudan site adresi yerine kendi sunucumuzdaki proxy rotasına yönlendiriyoruz
...
// İframe içinden gelen kaydırma (scroll) hareketlerini yakalama
...
// Diğer kullanıcıdan gelen kaydırma hareketini uygulama
...
sendFriendReqBtn.addEventListener('click', () => {
    ...
    if (friendName) {
        socket.emit('send-friend-request', { to: friendName, from: currentUsername });
...
sendDmBtn.addEventListener('click', () => {
    ...
    if (msg && currentDmUser) {
        socket.emit('send-private-message', { to: currentDmUser, text: msg, from: currentUsername });
...
socket.on('receive-private-message', (data) => {
    if (currentDmUser === data.from) {
        appendDmMessage(data.text, false);
    } else {
...
socket.on('friend-request-received', ({ from }) => {
    ...
    div.innerHTML = `<span>${from}</span> <button onclick="acceptFriend('${from}')" style="background: #248046; color: white; border: none; padding: 3px 6px; border-radius: 3px; cursor: pointer;">Kabul Et</button>`;
    reqList.appendChild(div);
});

window.acceptFriend = function(fromUsername) {
    socket.emit('accept-friend-request', { from: fromUsername, to: currentUsername });
};
