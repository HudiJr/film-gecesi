...
function loadAndPlayVideo(url) {
    ...
    mainVideo.src = url;
    mainVideo.load();
    mainVideo.play().catch(e => console.log("Otomatik oynatma engeli:", e));
}

// Video Linki Yükleme (Butona Basıldığında)
loadVideoBtn.addEventListener('click', () => {
    ...
    if (url) {
        loadAndPlayVideo(url); // Kendi ekranında aç
        socket.emit('video-action', { roomId: currentRoom, type: 'change-src', url: url }); // Karşı tarafa gönder
    }
});

// Video Hareketlerini Dinleme
