...
const rooms = {};
// Kullanıcıların online durumlarını, arkadaş listelerini ve DM'lerini tutmak için hafıza tabanı
...
io.on('connection', (socket) => {
    ...
    socket.on('register-user', ({ username }) => {
        ...
        if (!usersDB[username]) {
            usersDB[username] = { socketId: socket.id, friends: [], requests: [], messages: {} };
        } else {
    ...
    socket.on('send-friend-request', ({ targetUsername }) => {
        ...
        if (targetUser && targetUsername !== socket.username) {
            if (!targetUser.requests.includes(socket.username) && !usersDB[socket.username].friends.includes(targetUsername)) {
    ...
    socket.on('send-private-message', ({ targetUsername, message }) => {
        ...
        // Alıcının DM geçmişine ekle ve online ise ilet
        if (target) {
